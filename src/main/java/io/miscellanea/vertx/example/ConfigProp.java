package io.miscellanea.vertx.example;

/**
 * Constants naming configuration properties.
 *
 * @author Jason Hallford
 */
public final class ConfigProp {
  public static final String KEYS = "keys";
  public static final String PUBLIC_KEY = "public";
  public static final String IDP_ALGORITHM = "idp-algorithm";
  public static final String CONFIG_FILE = "rp-config-file";
  public static final String BIND_PORT = "bind-port";
  public static final String KEY_STORE = "rp-keystore";
  public static final String KEY_STORE_PASSWORD = "rp-keystore-password";

  private ConfigProp() {
  }
}
